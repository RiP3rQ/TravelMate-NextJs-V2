...
export default async function handler(req, res) {
  ...
  const { rating, title, description, imageSrc, attractionId } = req.body;
  ...
  if (!rating || !title || !description || !attractionId) {
    return res.status(400).json({ message: "Missing required fields" });
  }

  // Create a new review for the given listing with the given rating
  const review = await prisma.Attraction_Review.create({
    data: {
  ...
  const reviews = await prisma.Attraction_Review.findMany({
    where: {
